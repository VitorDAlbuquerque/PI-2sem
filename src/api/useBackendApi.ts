...
export const useBackendApi = () => ({
  ...
  newFollow: async(token: string, followedId: string)=>{
    const response = await axios.post(`${URLHost}/newFollow`, {followedId},{
      headers:{
        Authorization: `Bearer ${token}`
      }
    })
  ...
  listFollowers: async(userId: string)=>{
    const response = await axios.get(`${URLHost}/listFollowers/${userId}`)
    return{
      follow: response.data
    }
  },
  listFollowersByName: async(userId: string, name: string)=>{
    const response = await axios.post(`${URLHost}/listFollowersByName`, {userId, name})
    return{
      follow: response.data
    }
  },
