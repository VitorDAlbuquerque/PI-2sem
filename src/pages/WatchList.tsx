                    ...
                    <div className="p-10">
                        ...
                        <div>
                            <div className="flex gap-4 items-center text-gray-400 mb-5">
                                Criado por: 
                                <p onClick={()=>navigate(`/Profile/${watchList.user.id}`)}  className="flex items-center gap-2 hover:text-constrastColor cursor-pointer hover:underline"><img className="h-7 w-7 object-cover rounded-full" src={profileImgs? profileImgs[watchList.user.imgIndex].url : ''} alt="" />{watchList.user.name}</p>
                                
                                {watchList.isLiked.length > 0?
                                    watchList.isLiked.filter((like) => like.userId == authContext.user?.id).length> 0?
                                        <p key={watchList.user.id} onClick={newLike} className="flex items-center gap-2 text-xl  cursor-pointer hover:brightness-75 transition-all duration-200"><b className="text-constrastColor"><FaHeart/></b>{watchList.isLiked.length}</p>
