        ...
        <div className="flex items-center flex-col">
          ...
          <div className="w-full flex flex-col justify-center items-center min-h-28 max-w-full py-6 bg-bgAside">
            <div className="flex items-center mb-6">
              ...
              <input type="text" id="inputSearch" onChange={searchMovie} className="w-[500px] h-12 bg-slate-900 border-2 text-slate-400 border-slate-400 rounded-md px-10  outline-none focus:border-constrastColor" placeholder="Procurar filmes"/>
            </div>
            ...
            <div className="w-full flex justify-center items-center gap-2 flex-wrap text-slate-400 relative">
              <Carousel className="w-full max-w-5xl flex items-center" opts={{ dragFree: true }} setApi={setCarouselApi}>
                    ...
                    {genres.map(genre =>{
                      return(
                        <CarouselItem key={genre.id} className="basis-1/7">
                          <div
                          className="border-2 border-slate-400 bg-slate-900 flex justify-center rounded-md hover:border-constrastColor transition-all duration-300 cursor-pointer min-w-[137px] "
                          onClick={()=>filterByGenre(String(genre.id), String(page))}
                          >
                            <ToggleGroupItem className="h-full w-full rounded-sm p-1" value={String(genre.id)}>{genre.name}</ToggleGroupItem>
                          </div>
                        </CarouselItem>
                      )
                    })}
                ...
                <button onClick={()=>dotCarousel(-5)} className="text-gray-300 text-3xl absolute -left-10 hover:bg-mainBgOpacity75 h-full transition-all ease-in-out duration-200 group-hover:block">
                  <IoIosArrowBack />
                </button>
                <button onClick={()=>dotCarousel(+4)}  className="text-gray-300 text-3xl absolute -right-10 hover:bg-mainBgOpacity75 h-full transition-all ease-in-out duration-200 group-hover:block">
                  <IoIosArrowForward />
                </button>
              </Carousel>
            </div>
            ...
            <PaginationContent className="text-white">
              ...
              {currentPage==1?
                <div className="flex gap-1">
                  ...
                  <PaginationItem >
                    <PaginationLink className="text-black" onClick={()=>{
                      {genre?
