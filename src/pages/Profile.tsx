...
export function Profile() {
  ...
  async function admUpdatePassword(e: FormEvent) {
    ...
    if (storageData && profileUser) {
      try {
        ...
        toast.error("Ocorreu um erro ao atualizar a senha. Por favor, tente novamente mais tarde.");
      }
    } else {
  ...
  async function admBan(){
    ...
    if(storageData && profileUser){
      ...
      navigate('/')
      console.log('Usuário foi banido com sucesso.')
      toast.success('Usuário foi banido com sucesso.')
    } else {
      console.log ('Usuário não foi banido. Tente novamente.')
      toast.error ('Usuário não foi banido. Tente novamente.')
    }
  }
              ...
              <div className="flex flex-col gap-1 mt-5 ">
                      ...
                      <PopoverContent className="p-2 max-w-44 dark:bg-black dark:border-2 dark:border-white">
                        <Dialog onOpenChange={setOpenDialogUpdatePassword} open={openDialogUpdatePassword}>
                          <DialogTrigger className="w-full text-left">
                            <p className="cursor-pointer hover:brightness-90 bg-white px-2 rounded-sm transition-all duration-200 dark:bg-black dark:text-white">Alterar senha</p>
                          </DialogTrigger>
                          <DialogContent className="max-w-96 rounded-lg dark:text-white dark:bg-black dark:border-2 dark:border-white ">
                            <DialogHeader>
                              <DialogTitle className="dark:bg-black dark:text-white">Alterar senha de {profileUser.name}</DialogTitle>
                              <DialogDescription >
                        ...
                        <Dialog onOpenChange={setOpenDialogBan} open={openDialogBan}>
                          <DialogTrigger className="w-full text-left">
                            <div className="cursor-pointer hover:bg-red-200 hover:text-red-500 bg-white px-2 rounded-sm transition-all duration-200 dark:bg-black">
                ...
                <Dialog>
                  <DialogTrigger>
                    <p
                      id="followers"
                      className="font-light invert text-left  mobile:invert-0 cursor-pointer hover:underline dark:text-white "
                    >
                      {followers?
                        followers.length
                      :null} Seguidores
                  ...
                  <DialogContent className="max-w-96 rounded-lg text-left max-h-96 dark:bg-black dark:border-2 dark:border-white">
                    <DialogHeader>
                      <DialogTitle className="dark:text-white ">Seguidores</DialogTitle>
                    </DialogHeader>
                    <div className="flex flex-col gap-5">
                      <input className="mt-2 h-9 border-2 rounded-md outline-none px-2 dark:bg-black dark:border-2 dark:border-white" type="text" onChange={(e)=>listFollowersByName(e.target.value)} placeholder="Pesquisar"/>
                      <ScrollArea className="h-48">
                        <div className="flex flex-col gap-5">
                          {followers.length > 0?
                            followers.map(follow =>{
                              return(
                                <div key={`${follow.userFollowingId}${follow.userFollowedId}`} className="flex items-center gap-3 cursor-pointer dark:text-white">
                          ...
                          : 
                            <div className="w-full h-full flex justify-center items-center min-h-24 dark:text-white">
                ...
                <p className="font-light max-w-80 mt-3 min-h-28 dark:text-white">
                  {profileUser?.bio}
            ...
            {authContext.user && profileUser?
              profileUser.id != authContext.user?.id?
                ...
                <div className="h-2/5 absolute right-5 bottom-0">
                  <button className="font-semibold bg-gray-700 py-1 px-4 rounded-md text-gray-400  hover:brightness-75 transition-all duration-200 dark:bg-black dark:border:white dark:text-white dark:border-2" onClick={newFollow}>Deixar de seguir</button>

                </div>
                :
                <div className="h-2/5 absolute right-5 bottom-0">
                  <button className="font-semibold bg-darkGreen py-1 px-4 rounded-md text-constrastColor hover:bg-constrastColor hover:text-darkGreen transition-all duration-200  dark:border-yellow-400 dark:bg-yellow-400 dark:hover:text-black dark:text-black " onClick={newFollow}>Seguir</button>
                </div>  
              :null
            :null
            }
