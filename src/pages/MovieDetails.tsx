...
export function MovieDetails() {
  ...
  return (
    ...
    <div className="dark:bg-black bg-mainBg flex-initial w-full min-h-screen font-montserrat">
      <Header />
        <div className='{`mx-10 my-8 p-10 flex mobile:flex-col mobile:items-center  flex-row"}`}'>
          <div className={`flex-none mx-5 mr-8  mobile:mb-8  mobile:w-auto mobile:mr-8 "}`}>
            <img
          ...
          <div className="flex-grow max-w-full text-mainFontColor">
          <div className="flex flex-col md:flex-row md:items-center">
            <div className="flex flex-col md:flex-row md:items-center md:max-w-full">
          ...
          {movieDetails?.release_date && (
            ...
            <div className="flex items-center md:items-center mb-8 mt-2 ">
              ...
              <Dialog open={newMovieAtListDialog} onOpenChange={setNewMovieAtListDialog}>
                <DialogTrigger>
                  <div className="flex items-center ">
                    <p className="text-slate-400 hover:text-lightGreen dark:text-yellow-300  cursor-pointer transition-all ease-in-out duration-200 mx-10 " >
                ...
                <div className="text-sm text-slate-500 flex flex-col gap-4 dark:text-white">
                  <div>
                    ...
                    <select value={watchlistId} onChange={(e)=>{
                      ...
                      }} className=" max-w-full h-10 border rounded-md dark:text-white dark:bg-black dark:border-2 dark:border-white 
                      ">
      ...
      <DialogClose onClick={()=>{
       ...
       <button onClick={()=>addMoviesOnWatchlist()} className="border-2 w-20 bg-white border-constrastColor p-2 rounded-lg hover:brightness-90 transition-all  duration-200 dark:border-yellow-400 dark:bg-black dark:text-white">
          Adicionar
            ...
            <div className="text-text-sm mb-2 mobile:text-center">
            ...
            <div className='text-lg my-6 max-w-4xl mobile:text-center'>{movieDetails?.overview}</div>
          </div>
        </div>
  
        <div className="mx-auto text-gray-500 text-xl font-semibold flex justify-center flex-wrap">
      ...
      authContext.user?.id != userComment?.userId? (
        <div className="flex items-center mb-12 gap-4 ">
          ...
          <div className="flex flex-col gap-2 ">
            <div className="h-10 text-gray-400 px-3 py-6 bg-slate-900 hover:border-constrastColor transition-all duration-200 border-2 border-slate-400 rounded-sm justify-center flex items-center dark:text-white dark:hover:border-yellow-400 dark:bg-black dark:border-white ">
              ...
              {[1, 2, 3, 4, 5].map((index) => (
                <button
                  key={index}
                  type="button"
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${rating >= index ? 'text-constrastColor dark:text-yellow-400' : 'text-gray-400 dark:text-white '}`}
            ...
            <input
              name="text"
              className="px-3 h-10 max-w-full py-8 rounded-sm outline-none border-2 border-slate-400 bg-slate-900 text-slate-400 focus:border-constrastColor transition-all duration-200 dark:focus:border-yellow-400 dark:bg-black  dark:border-white"
              type="text"
              defaultValue={userComment?.text}
              placeholder="Adicione um novo comentário..."
            />
          </div>
            <button  type="submit" className="h-10 text-gray-400 px-3 hover:border-constrastColor transition-all duration-200 border-2 border-slate-400 rounded-sm justify-center flex items-center mt-4 dark:text-white dark:border-white">Avaliar</button>
          </form>
      ...
      <div >
        {comments.map((comment) => {
          if(comment.userId == authContext.user?.id){
            if(!alterComment){
              return (
                <div key={`${comment.userId}${comment.movieId}`} className="mb-6 flex flex-col gap-2 dark:bg-black max-w-md">
                    <div className="flex items-center gap-2 text-gray-400 dark:text-white ">
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="p-2 max-w-44 dark:bg-black dark:border-2 dark:border-white">
                            <div className="cursor-pointer hover:bg-gray-400 rounded-sm transition-all duration-200 p-1 dark:bg-black dark:hover:opacity-85 max-w-full" onClick={()=>{
                              setAlterCommet(true)
                            ...
                            <Dialog >
                              ...
                              <DialogContent className="max-w-max rounded-lg xs:w-2 mobile:p-4 dark:text-white dark:bg-black dark:border-2 dark:border-white 
">
                                <DialogHeader>
                    ...
                    <p className="relative ml-12 flex gap-2 items-center text-gray-400  dark:text-white">{comment.rating}/5 <b className="text-constrastColor dark:text-yellow-400"><GiKiwiFruit/></b></p>
                              {/*acho que é aqui o erro*/}
                    <p className="relative ml-12 text-gray-400 comment-text  max-w-fit mr-8  break-words pr-5 ">{comment.text}</p>
          ...
          <img className="h-12 w-12 rounded-full object-cover" src={profileImgs[comment.user.imgIndex].url} />
          ...
          <div className="flex flex-col gap-2 ">
            <div className="h-10 text-gray-400 px-3 py-6 bg-slate-900 hover:border-constrastColor transition-all duration-200 border-2 border-slate-400 rounded-sm justify-center flex items-center dark:bg-black dark:text-white dark:hover:border-yellow-400 dark:border-white">
             ...
             <input
              name="text"
              className="px-3 h-10 max-w-full py-8 rounded-sm outline-none border-2 border-slate-400 bg-slate-900 text-slate-400 focus:border-constrastColor transition-all duration-200 dark:bg-black dark:text-white dark:hover:border-yellow-400 dark:border-white"
          ...
          <div className="flex gap-5">
            ...
            return (
              <div key={`${comment.userId}${comment.movieId}`} className="mb-6 ">
                  ...
                  <p className="relative ml-12 text-gray-400 comment-text max-w-full break-words">{comment.text}</p>
