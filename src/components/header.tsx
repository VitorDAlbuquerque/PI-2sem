  ...
  return (
    <div className="flex justify-between items-center h-20 bg-bgAside w-full  relative py-14 dark:bg-black group mobile:group-focus-within:justify-center ">
      ...
      <div className="items-center relative flex right-12 ">
        ...
        <div className={`bg-slate-200 border-2 border-slate-600 w-full absolute top-10 rounded-lg dark:bg-black dark:border-white dark:text-white ${text? null : "hidden"} z-50 flex flex-col gap-2`}>
          {users.map(user =>{
            return(
              <div key={user.id} className="flex gap-2 cursor-pointer hover:brightness-90 transition-all duration-200 bg-slate-200 px-2 py-1 rounded-lg line-clamp-1 dark:bg-black  " onClick={()=>navigate(`/Profile/${user.id}`)}>
